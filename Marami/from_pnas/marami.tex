...
Plusieurs modeles de prédictions de liens puissant ont été proposé dans litterature pour apporté une solution du probléme dit de \textit{prédiction de liens} consistant a prédire la probabilité d'une relation entre deux noeud quelconque dans un réseaux \cite{LibenNowell07, HassanZaki11}.
Parmis ces modéles, la class des  modéles \mmm a recu une attention particulière  due a leurs  capacité a faire emerger la structure d'un réseaux et d'inférer des liens non observés.
Deux modéles principaux, repésantant cette classe, ont été proposés et étudiés  dans la littérature: le modèle \textcolor{red}{de bloc stochastique a appartenance multiple (ouah, ca le fait pas trop en francais je trouve :') )} (MMSB) \cite{MMSB} et son extension non-paramétrique ~\cite{iMMSB, fan2015dynamic} d'un coté et le modéle à attribute latents (ILFM) \cite{BMF} et son extension non-paramétrique \cite{ILFRM} de l'autre.


Les modéles probabilistes, tel que ceux considérés sont usuellement evalué empiriquement. Néanmoins, les résaux sociaux rééls exhibent des propriétés générales qui permettent de les charactériser. Dans ce papier, nous traitons deux propriétés importantes, \textit{l'homophilie} et \textit{l'attachement prédérentielle} \cite{Newman2010, Barabasi2003}. Nous étudions dans quelle mesures les modéles de prédictions de liens, de la classe mentionné, se conforment à elles. 

L'homophilie est une proriété qui correspond à l'idée que la probabilité que deux noeuds du réseaux soient liés augmente avec leurs similarités et comunnément suggérer par la locution \emph{"qui se ressemble s'assemble"}.

L'attachement préférentielle est une proriété d'avantage "topologique" dans le sens ou les ne dépend que du nombre de connections présentes dans le réseaux. Elle signifie que la probalité de créer un liens avec un autre noeud dépend du nombre de liens de ce dernier (son degré). Cette proriété est d'une certaine facon, l'image de l'aphorisme populaire \emph{"On ne prête qu'au riche"}. En théorie des graphes l'attachement préférentiel est evoqué pour expliquer l'émergence de graph "scale-free" qui sont carachtérisé par une distribution des degrés en loi de puissance.

L'intéret de ces proriétés dans l'analyse des réseaux sociaux a été largement mise en avant pour modélisation de réseaux et l'amélioration des taches de prédictions de liens comme de détection de communauté.
...
\section{Modèles de prédictions de liens stochastic}
