...
Recently,  several extensions of \imb ~and \ifm  ~have been proposed. In \cite{AMMSB}, the authors  proposes an adaptation of \imb, called a-MMSB where "a" stands for assortative,  and they used it for discovering overlapping communities in large size networks having millions of nodes since a-MMSB  scales well using stochastic variational inference. They constrained the weight matrix to have weights in the diagonal and a fixed small value elsewhere. A non parametric dynamic version of MMSB model has also been introduced to  handle temporal networks \cite{fan2015dynamic}. \ifm has also been extended in several way, to handle non-negative weights in \cite{IMRM} and within a more subtle latent feature structure in \cite{ILAM}. Nevertheless, the characterization of these models with regards to the properties of networks remains to be explored \cite{jacobs2014unified}.

In the Multiplicative Attribute Graph (MAG) model \cite{Kim2012} which considers nodes affinities  or the Dancer model which takes in consideration both properties \cite{Largeron2017} but this model do not belong to the family of Bayesian models considered in this article. These properties has also  been exploited for improving the results obtained in classical tasks such as community detection \cite{Ciglan2013,Zhang2016} or link prediction \cite{Aiello2012,Zeng2016}. However, few works studied, in a theoretical way, in which extend the models comply these properties.

Concerning the preferential attachment, Orbanz and Royhowever pointed out that models belonging to the family of infinitely exchangeable Bayesian graph models cannot generate sparse networks which not compatible with power law degree distribution among the vertices \cite{orbanz2015bayesian}. By relaxing the exchangeability assumptions, Lee \textit{et al.} \cite{Lee2015} analytically prove that their random model preserves the power law behavior and show that it satisfies the small world property. However the complexity of the model makes it hard to use in practice.

Concerning the homophily effect, \cite{hoff2008modeling} pointed out that the latent eigen model can comply both homophily and stochastic equivalence in undirected graphs but without providing a formal definitions of these properties. Furthermore, according to Lee \textit{et al.}, the latent eigen model, MLFM, for directed graphs fails to model homophily  and, for enhancing homophily modeling in MLFM, these last ones designed the  GLFM model \cite{Li11}.
