...
Nevertheless, although drawn from a wide range of domains, real world social networks exhibit general properties and one can wonder if these models are able to capture these properties. For instance, we can mentioned the small world property which implies that most vertices can be reached from every other through a small number of edges or the community structure which appears when vertices are grouped in a way such that vertices in a group are more connected to vertices belonging to the same group compared to other vertices.
 In this work, we focus on two other important properties, namely \textit{homophily} and \textit{preferential attachment} \cite{Newman2010, Barabasi2003} and assess to which extent link prediction models, as the ones mentioned above, comply them.
Homophily is verified in a network when similar vertices according to their properties tend to be more connected than dissimilar vertices.  Preferential attachment states that a vertex is more likely to create connections with vertices having a high degree.  It leads to networks where the degree distribution can usually be modeled by a power law.

// to complet ~\\
\textbf{A dev Interet de la classe de models etudies}


// to complete ~\\
The rest of this paper is organized as follows: in the next section, related works are presented and in the third section we introduce the  background of the stochastic mixed membership models . In the  fourth and fifth sections we introduce formal definitions of the compliance of the models to homophily and preferential attachment in social networks, and we study how the models comply or not with those properties. In the sixth section, we illustrate our findings on 2 synthetic networks and 2 real world networks. Finally, we conclude in the last section.
