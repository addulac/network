...
Let us assume that elements of $\mat{\hat{F}}$ follow a power law distribution, \textit{i.e.} that for all $i$ and $k$, $pdf(\hat{f}_{ik}) \propto x^{- \alpha_{ik}}$ with $\alpha_{ik} > 1$ where $pdf$ denotes the probability density function. Then \imb\ satisfies the local preferential attachment effect in $\M_e$.
\end{proposition}
