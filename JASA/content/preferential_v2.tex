...
Let us assume that elements of $\mat{\hat{F}}$ follow a power law distribution, \textit{i.e.} that for all $i$ and $k$, $\hat{f}_{ik} \sim L(x) x^{- \alpha_{ik} + 1}$ where $L(x)$ is a slow varying function and $\alpha_{ik} > 1$. Then \imb\ satisfies the local preferential attachment effect in $\M_e$.
\end{proposition}
