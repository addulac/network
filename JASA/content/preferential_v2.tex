...
\begin{equation*}
x^{a_{ik}-1}\left(a_{ik}(1-\epsilon\frac{E_{d_{ik}}}{f_{ik}}) -x(a_{ik}-b_{ik})\frac{f_{ik}}{E_{d_{ik}}} \right) \geq \frac{b_{ik}}{F_k^N(F_k^p)^{a_{ik}-2}\phi_{kk}^{a_{ik}-1}}
\end{equation*}
...
As one can note, when $x$ is large, $\epsilon$ is small $b-a>0$ (which roughly means that the class $k$ concentrates less that than half of the capacaity of the network.) then the above condition is likely to be met and \imb satisfies the local preferential attachment effect. 
...
\end{align*}
where $q=\frac{F_{ik}^p}{F_{ik}^N}$. The conditional distribution $g(x)$ is not trivially equal to one in the case where $qx+\epsilon > x \Leftrightarrow \epsilon > x(1-q)$. Further, the survival function of $\hat f_{ik}$ is $P(\hat f_{ik} >x) = 1-\int_0^x f(x)$ where $f(x)$ is the density of $\hat f_{ik}$. On can show that the marginal distribution of $\hat f_{ik}$ is a Beta  of the form $f(x)=\Beta(a_{ik}, b_{ik})$  where $a_{ik} = \alpha_0\beta_k + N_{ik}$ and $b_{ik} = \sum_{k'\neq k} \alpha_0\beta_{k'} + N_{ik'}$  (this is a consequence of the form of the posterior distribution of the DP). In the following we ommit the references to $i$ and $k$ for the parameters of the Beta, simply noting $\Beta(a, b)$ for short. The derivative og $g$ is
\begin{equation*}
...
\begin{equation*}
x'^{a-1}\left(a(1-\epsilon'\frac{E_{d_{ik}}}{f_{ik}}) -x'(a-b)\frac{f_{ik}}{E_{d_{ik}}} \right) \geq \frac{b}{F_k^N(F_k^p)^{a-2}\phi_{kk}^{a-1}}
\end{equation*}
