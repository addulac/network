...
\end{align}
%
as $\hat{\phi}_{kk} = 0$. Let us now consider $\mat{\hat{f}}_i=\mat{\hat{f}}_j=(0,1,0)$ and $\mat{\hat{f}}_{i'}=(1,0,1)$ and $\mat{\hat{f}}_{j'}=(0,1,0)$. Then, $s_l(i,j)=1$ and $s_l(i',j')=0$. However, $\pr(y_{ij}=1 \mid \M_e) = 0$ whereas $\pr(y_{ij}=1 \mid \M_e) > 0$. \imb\ is thus not homophilic under $s_l$. The same example can be used to show that \ifm\ is neither homophilic under $s_l$.
...
As mentioned before, in the context of social networks, the notion of preferential attachement appears at different levels: (a) at a global level that characterizes the degree distribution of nodes in the network, and (b) at a local level that characterizes the degree distribution of nodes within classes. These two phenomena simply state that the more links a node has, either globally in the network or within a particular class, the more likely it will be linked to new nodes, globally in the network or locally in its class. For global preferential attachment, the degree of a node is a known integer; for local preferential attachment, in most models, the exact degree is not known, and one has to rely on an estimate. We provide below a formal definition of these elements.
%
...
\end{definition}
%
As one can note, these definitions directly translate the fact that "the more connections a node has, the more likely it is to be connected to new nodes". The only difference between the local and global cases lies in the fact the degree is usuallly unknown in the local case, and is here estimated through its expectation.
...
\]
%
where $|.|$ denotes the cardinal of a set. The local degree for \ifm\ thus amounts to the number of nodes connected to $i$ that belong to latent factor $k$, weighted by $\sigma(\hat{\Phi}_{kk})$. As $\sigma(\hat{\Phi}_{kk})$ is strictly positive,  $\pr(d_{i,k} \ge x+\epsilon \mid d_{i,k} \ge x, \mathcal{M}_e) = \pr(|\{j \in \mathcal{V}(i), \hat{f}_{jk} =1\}| \ge x+\epsilon \mid |\{j \in \mathcal{V}(i), \hat{f}_{jk} =1\}| \ge x, \mathcal{M}_e)$. The reasoning for global preferential attachement thus applies here, showing that \imb\ does not satisfy the local preferential attachment effect. \hspace{7.1cm} $\Box$

The above propositions show that both models are deficient in the sense that they do not guarantee that the networks they generate will comply to the global (and local in case of \ifm) preferential attachment phenomena, which are \textit{inherent} properties of the probability distributions underlying the models. This does not mean however that \ifm\ and \imb\ ISMMB are not able to model well social networks during the learning phase, even according to the underlying degree distribution. Indeed, the Gibbs updates for both models will assign higher weight to nodes and factors that have been used during the learning phase. Provided there is enough training data, both models will likely reproduce the degree distributions observed in the training data. We will observe that in the following section, devoted to the illustration of the properties we have established.
